...
import { NoEffect, PerformedWork, Placement, Update, Snapshot, PlacementAndUpdate, Deletion, ContentReset, Callback, DidCapture, Ref, Incomplete, HostEffectMask } from 'shared/ReactSideEffectTags';
import { HostRoot, ClassComponent, ClassComponentLazy, HostComponent, ContextProvider, HostPortal } from 'shared/ReactWorkTags';
import { enableSchedulerTracing, enableProfilerTimer, enableUserTimingAPI, replayFailedUnitOfWorkWithInvokeGuardedCallback, warnAboutDeprecatedLifecycles, warnAboutLegacyContextAPI, enableSuspense } from 'shared/ReactFeatureFlags';
...
import { now, scheduleDeferredCallback, cancelDeferredCallback, prepareForCommit, resetAfterCommit } from './ReactFiberHostConfig';
import { markPendingPriorityLevel, markCommittedPriorityLevels, markSuspendedPriorityLevel, markPingedPriorityLevel, hasLowerPriorityWork, isPriorityLevelSuspended, findEarliestOutstandingPriorityLevel, didExpireAtExpirationTime } from './ReactFiberPendingPriority';
...
import { NoWork, Sync, Never, msToExpirationTime, expirationTimeToMs, computeAsyncExpiration, computeInteractiveExpiration } from './ReactFiberExpirationTime';
import { AsyncMode, ProfileMode } from './ReactTypeOfMode';
import { enqueueUpdate, resetCurrentlyProcessingQueue } from './ReactUpdateQueue';
import { createCapturedValue } from './ReactCapturedValue';
import { isContextProvider as isLegacyContextProvider, popTopLevelContextObject as popTopLevelLegacyContextObject, popContext as popLegacyContext } from './ReactFiberContext';
...
import { throwException, unwindWork, unwindInterruptedWork, createRootErrorUpdate, createClassErrorUpdate } from './ReactFiberUnwindWork';
import { commitBeforeMutationLifeCycles, commitResetTextContent, commitPlacement, commitDeletion, commitWork, commitLifeCycles, commitAttachRef, commitDetachRef } from './ReactFiberCommitWork';
...
if (enableSchedulerTracing) {
  ...
  invariant(__interactionsRef != null && __interactionsRef.current != null, 'It is not supported to run the profiling version of a renderer (for example, `react-dom/profiling`) ' + 'without also replacing the `schedule/tracing` module with `schedule/tracing-profiling`. ' + 'Your bundler might have a setting for aliasing both modules. ' + 'Learn more at http://fb.me/react-profiling');
}
...
let lastUniqueAsyncExpiration = 0; // Represents the expiration time that incoming updates should use. (If this
...
let interruptedBy = null; // Do not decrement interaction counts in the event of suspense timeouts.
// This would lead to prematurely calling the interaction-complete hook.

let suspenseDidTimeout = false;
...
function commitBeforeMutationLifecycles() {
  try {
    console.group("commitBeforeMutationLifecycles")
    ...
    console.groupEnd("commitBeforeMutationLifecycles");
  }
}

function commitAllLifeCycles(finishedRoot, committedExpirationTime) {
  try {
    console.group("commitAllLifeCycles")
    ...
    while (nextEffect !== null) {
      ...
      const next = nextEffect.nextEffect; // Ensure that we clean these up so that we don't accidentally keep them.
    ...
    console.groupEnd("commitAllLifeCycles");
  }
}

function isAlreadyFailedLegacyErrorBoundary(instance) {
  try {
    console.group("isAlreadyFailedLegacyErrorBoundary")
    return legacyErrorBoundariesThatAlreadyFailed !== null && legacyErrorBoundariesThatAlreadyFailed.has(instance);
  } finally {
    console.groupEnd("isAlreadyFailedLegacyErrorBoundary");
  }
}

function markLegacyErrorBoundaryAsFailed(instance) {
  try {
    console.group("markLegacyErrorBoundaryAsFailed")
    ...
    console.groupEnd("markLegacyErrorBoundaryAsFailed");
  }
}
...
function commitRoot(root, finishedWork) {
  try {
    ...
    const earliestRemainingTimeBeforeCommit = updateExpirationTimeBeforeCommit === NoWork || childExpirationTimeBeforeCommit !== NoWork && childExpirationTimeBeforeCommit < updateExpirationTimeBeforeCommit ? childExpirationTimeBeforeCommit : updateExpirationTimeBeforeCommit;
    markCommittedPriorityLevels(root, earliestRemainingTimeBeforeCommit);
    let prevInteractions = null;
    let committedInteractions = enableSchedulerTracing ? [] : null;

    if (enableSchedulerTracing) {
      // Restore any pending interactions at this point,
      // So that cascading work triggered during the render phase will be accounted for.
      prevInteractions = __interactionsRef.current;
      __interactionsRef.current = root.memoizedInteractions; // We are potentially finished with the current batch of interactions.
    ...
    const earliestRemainingTimeAfterCommit = updateExpirationTimeAfterCommit === NoWork || childExpirationTimeAfterCommit !== NoWork && childExpirationTimeAfterCommit < updateExpirationTimeAfterCommit ? childExpirationTimeAfterCommit : updateExpirationTimeAfterCommit;
    ...
    console.groupEnd("commitRoot");
  }
}

function resetChildExpirationTime(workInProgress, renderTime) {
  try {
    console.group("resetChildExpirationTime")
    ...
    console.groupEnd("resetChildExpirationTime");
  }
}

function completeUnitOfWork(workInProgress) {
  try {
    console.group("completeUnitOfWork")
    ...
    console.groupEnd("completeUnitOfWork");
  }
}
...
function performUnitOfWork(workInProgress) {
  try {
    ...
    console.groupEnd("performUnitOfWork");
  }
}
...
function workLoop(isYieldy) {
  try {
    ...
    console.groupEnd("workLoop");
  }
}

function renderRoot(root, isYieldy, isExpired) {
  try {
    console.group("renderRoot")
...
function captureCommitPhaseError(fiber, error) {
  try {
    console.group("captureCommitPhaseError")
    return dispatch(fiber, error, Sync);
  } finally {
    console.groupEnd("captureCommitPhaseError");
  }
}

function computeThreadID(expirationTime, interactionThreadID) {
  try {
    console.group("computeThreadID")
    // Interaction threads are unique per root and expiration time.
    return expirationTime * 1000 + interactionThreadID;
  } finally {
    console.groupEnd("computeThreadID");
  }
...
function computeUniqueAsyncExpiration() {
  try {
    console.group("computeUniqueAsyncExpiration")
    ...
    console.groupEnd("computeUniqueAsyncExpiration");
  }
}

function computeExpirationForFiber(currentTime, fiber) {
  try {
    console.group("computeExpirationForFiber")
    let expirationTime;
    ...
    console.groupEnd("computeExpirationForFiber");
  }
}

function renderDidSuspend(root, absoluteTimeoutMs, suspendedTime) {
  try {
    console.group("renderDidSuspend")
    ...
    console.groupEnd("renderDidSuspend");
  }
}

function renderDidError() {
  try {
    console.group("renderDidError")
    nextRenderDidError = true;
  } finally {
    console.groupEnd("renderDidError");
  }
}

function retrySuspendedRoot(root, fiber, suspendedTime) {
  try {
    console.group("retrySuspendedRoot")
    ...
    if (enableSuspense) {
      ...
      if (rootExpirationTime !== NoWork) {
        if (enableSchedulerTracing) {
          ...
          __interactionsRef.current = root.memoizedInteractions; // Because suspense timeouts do not decrement the interaction count,
    ...
    console.groupEnd("retrySuspendedRoot");
  }
}

function scheduleWorkToRoot(fiber, expirationTime) {
  try {
    console.group("scheduleWorkToRoot")
...
function storeInteractionsForExpirationTime(root, expirationTime, updateInteractionCounts) {
  try {
    ...
    console.groupEnd("storeInteractionsForExpirationTime");
  }
}

function scheduleWork(fiber, expirationTime) {
  try {
    console.group("scheduleWork")
    ...
    if (root === null) {
      if (__DEV__ && (fiber.tag === ClassComponent || fiber.tag === ClassComponentLazy)) {
    ...
    console.groupEnd("scheduleWork");
  }
}

function deferredUpdates(fn) {
  try {
    console.group("deferredUpdates")
    const currentTime = requestCurrentTime();
    ...
    console.groupEnd("deferredUpdates");
  }
}

function syncUpdates(fn, a, b, c, d) {
  try {
    console.group("syncUpdates")
    const previousExpirationContext = expirationContext;
...
function recomputeCurrentRendererTime() {
  try {
    console.group("recomputeCurrentRendererTime")
    const currentTimeMs = now() - originalStartTimeMs;
    currentRendererTime = msToExpirationTime(currentTimeMs);
  } finally {
    console.groupEnd("recomputeCurrentRendererTime");
  }
}

function scheduleCallbackWithExpirationTime(root, expirationTime) {
  try {
    console.group("scheduleCallbackWithExpirationTime")
    ...
    console.groupEnd("scheduleCallbackWithExpirationTime");
  }
...
function onFatal(root) {
  try {
    console.group("onFatal")
    root.finishedWork = null;
  } finally {
    console.groupEnd("onFatal");
  }
}
...
function onComplete(root, finishedWork, expirationTime) {
  try {
    ...
    console.groupEnd("onComplete");
  }
}

function onSuspend(root, finishedWork, suspendedExpirationTime, rootExpirationTime, msUntilTimeout) {
  try {
    console.group("onSuspend")
    ...
    console.groupEnd("onSuspend");
  }
}

function onYield(root) {
  try {
    console.group("onYield")
    root.finishedWork = null;
  } finally {
    console.groupEnd("onYield");
  }
}

function onTimeout(root, finishedWork, suspendedExpirationTime) {
  try {
    console.group("onTimeout")
    ...
    console.groupEnd("onTimeout");
  }
}

function onCommit(root, expirationTime) {
  try {
    console.group("onCommit")
    root.expirationTime = expirationTime;
    root.finishedWork = null;
  } finally {
    console.groupEnd("onCommit");
  }
}

function requestCurrentTime() {
  try {
    console.group("requestCurrentTime")
    ...
    console.groupEnd("requestCurrentTime");
  }
...
function requestWork(root, expirationTime) {
  try {
    ...
    console.groupEnd("requestWork");
  }
}

function addRootToSchedule(root, expirationTime) {
  try {
    console.group("addRootToSchedule")
    ...
    console.groupEnd("addRootToSchedule");
  }
}

function findHighestPriorityRoot() {
  try {
    console.group("findHighestPriorityRoot")
    ...
    console.groupEnd("findHighestPriorityRoot");
  }
}

function performAsyncWork(dl) {
  try {
    console.group("performAsyncWork")
    ...
    console.groupEnd("performAsyncWork");
  }
}

function performSyncWork() {
  try {
    console.group("performSyncWork")
    performWork(Sync, null);
  } finally {
    console.groupEnd("performSyncWork");
  }
}

function performWork(minExpirationTime, dl) {
  try {
    console.group("performWork")
    ...
    console.groupEnd("performWork");
  }
}

function flushRoot(root, expirationTime) {
  try {
    console.group("flushRoot")
    ...
    console.groupEnd("flushRoot");
  }
}

function finishRendering() {
  try {
    console.group("finishRendering")
    nestedUpdateCount = 0;
    ...
    console.groupEnd("finishRendering");
  }
}

function performWorkOnRoot(root, expirationTime, isExpired) {
  try {
    console.group("performWorkOnRoot")
    ...
    console.groupEnd("performWorkOnRoot");
  }
}

function completeRoot(root, finishedWork, expirationTime) {
  try {
    console.group("completeRoot")
...
function onUncaughtError(error) {
  try {
    ...
    console.groupEnd("onUncaughtError");
  }
...
function batchedUpdates(fn, a) {
  try {
    ...
    console.groupEnd("batchedUpdates");
  }
} // TODO: Batching should be implemented at the renderer level, not inside
// the reconciler.
...
function unbatchedUpdates(fn, a) {
  try {
    console.group("unbatchedUpdates")
    ...
    console.groupEnd("unbatchedUpdates");
  }
...
function flushSync(fn, a) {
  try {
    ...
    console.groupEnd("flushSync");
  }
}

function interactiveUpdates(fn, a, b) {
  try {
    console.group("interactiveUpdates")
    ...
    console.groupEnd("interactiveUpdates");
  }
}

function flushInteractiveUpdates() {
  try {
    console.group("flushInteractiveUpdates")
    ...
    console.groupEnd("flushInteractiveUpdates");
  }
}

function flushControlled(fn) {
  try {
    console.group("flushControlled")
    const previousIsBatchingUpdates = isBatchingUpdates;
    isBatchingUpdates = true;
    ...
    console.groupEnd("flushControlled");
  }
}

export { requestCurrentTime, computeExpirationForFiber, captureCommitPhaseError, onUncaughtError, renderDidSuspend, renderDidError, retrySuspendedRoot, markLegacyErrorBoundaryAsFailed, isAlreadyFailedLegacyErrorBoundary, scheduleWork, requestWork, flushRoot, batchedUpdates, unbatchedUpdates, flushSync, flushControlled, deferredUpdates, syncUpdates, interactiveUpdates, flushInteractiveUpdates, computeUniqueAsyncExpiration };